        ...
        <div class="row" style="margin: 50px;">
            <div class="col-lg-6">
                ...
                <form action="{{route('tasks.update_description')}} " method="POST">
                    ...
                    <textarea class="form-control bg-light" id="ticket_description" name="ticket_description"
                              style="height: 14rem"></textarea>
                    <br>
            ...
            <div class="col-lg-6">
                    ...
                    <div class="col-sm-1">
                        <form action="{{route('tasks.update_draft')}}" method="POST">
                            ...
                            @if($ticket->is_draft == '1')
                                <input type="checkbox" id="draft" name="draft" value="draft" checked
                ...
                <form action="{{ route('tasks.update_assignee', ['ticket_id' => $ticket->id]) }}" method="POST"
                      name="assignee_change">
                    @csrf
                    <select class="form-select" style="width: 50%" id="selectField" name="assignee_select"
                            onchange="assignee_change.submit()">
                        ...
                        @if($ticket->is_completed == '0')
                            <form action="{{route('tasks.complete_ticket')}}" method="POST">
                                ...
                                <button type="submit" class="btn btn-success btn-sm float-end"
                                        onclick="return confirm('Are you sure you want to complete this ticket?')">
                                    Complete Ticket
                                </button>
                            </form>
                        @else
                            <form action="{{route('tasks.complete_ticket')}}" method="POST">
                                ...
                                <button type="submit" class="btn btn-warning btn-sm float-end"
                                        onclick="return confirm('Are you sure you want to return this ticket?')">Return
                                    Ticket
                                </button>
                            </form>
                        @endif
...
<script>
    ...
    window.onload = function () {
        getProjects('project_id'); //have to call it again, otherwise it doesn't popuplate
        populateTextArea();
    }
