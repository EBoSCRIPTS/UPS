        ...
        <div class="container" style="width: 80%">
            ...
            <table class="table">
                ...
                @foreach($projectMembers as $employee)
                    <div class="modal fade" id="performanceReportModal{{$employee->employee_id}}" tabindex="-1"
                         aria-labelledby="performanceReportModal" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title"
                                        id="performanceReportModal{{$employee->employee_id}}">{{$employee->employee->user->first_name}} {{$employee->employee->user->last_name}}
                                ...
                                <div class="modal-body">
                                    <form
                                        action="{{ route('tasks.performance_report_create', ['project_id' => $projectId]) }}"
                                        method="post">
                                        @Csrf
                                        <input type="hidden" name="date" value="{{Carbon\Carbon::now()}}">

                                        <textarea class="form-control" name="performance_report" id="performance_report"
                                                  col="30" rows="10"></textarea>
                                        <input class="form-select mt-3" type="number" name="performance_rating"
                                               id="performance_rating" min="1" max="100" placeholder="1-100">

                                        <input type="hidden" name="employee_id" id="employee_id"
                                               value="{{$employee->employee_id}}">
                                        <button type="submit" class="btn btn-primary float-end mt-3">Save changes
                                        </button>
                                    </form>
                                </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-success" data-bs-toggle="modal"
                                    data-bs-target="#performanceReportModal{{$employee->employee_id}}">Create a
                                performance report
                            </button>
                        </td>
            ...
            <a href="/tasks/projects/{{$projectId}}/performance_report/generate_xlsx" class="btn btn-success">Generate
                XLSX</a>
            <hr>
            <div class="row">
                <div class="col-md-6">
                        ...
                        <ul class="dropdown-menu">
                            @foreach($projectMembers as $employee)
                                <li style="margin-left: 15px"><input type="checkbox" class="type-checkbox"
                                                                     value="{{$employee->employee->user->first_name}} {{$employee->employee->user->last_name}}">{{$employee->employee->user->first_name}} {{$employee->employee->user->last_name}}
                                </li>
                            @endforeach
                        </ul>
                    </div>
            ...
            <table class="table table-striped" id="tableRatings">
                ...
                @foreach($reportedMonthPerformance as $performance)
                    <tr>
                        ...
                        <td><a href="/tasks/projects/{{$projectId}}/performance_report/soft_delete/{{$performance->id}}"
